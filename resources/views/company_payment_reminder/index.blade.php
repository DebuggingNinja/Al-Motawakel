                  ...
                  @else
                    @foreach ($paymentReminders as $paymentReminder)
                      <tr>
                        ...
                        <td>
                          <div class="userDatatable-content">
                            @if(($paymentReminder->company?->activeAccount?->total_rmb ?? 0) > 0)
                              <span class="text-danger fw-bold">
                          {{
                          __(
                          'company.We owe the company :amount RMB'
                          ,['amount'=>number_format(($paymentReminder->company?->activeAccount?->total_rmb ?? 0)
                          *-1, 2, '.', ',')]) }}
                        </span>
                            @elseif(($paymentReminder->company?->activeAccount?->total_rmb ??0) == 0)

                              0.00
                            @else
                              <span class="text-success fw-bold">
                          {{ str_replace('-', '', __('company.The company owe us :amount RMB',[
                              'amount'=>number_format(abs($paymentReminder->company?->activeAccount?->total_rmb ?? 0),
                          2, '.', ',')])) }}
                        </span>
                            @endif
                          </div>
                        ...
                        <td>
                          <div class="userDatatable-content">
                            @if(($paymentReminder->company?->activeAccount?->total_dollar ?? 0) > 0)
                              <span class="text-danger fw-bold">
                          {{
                          __(
                          'company.We owe the company :amount dollar'
                          ,['amount'=>number_format(($paymentReminder->company?->activeAccount?->total_dollar ?? 0)
                          *-1, 2, '.', ',')]) }}
                        </span>
                            @elseif(($paymentReminder->company?->activeAccount?->total_dollar ??0) == 0)

                              0.00
                            @else
                              <span class="text-success fw-bold">
                          {{ str_replace('-', '', __('company.The company owe us :amount dollar',[
                              'amount'=>number_format(abs($paymentReminder->company?->activeAccount?->total_dollar ?? 0),
                          2, '.', ',')])) }}
                        </span>
                            @endif
                          </div>
                       ...
                       d-inline-block w-auto px-2">
                        ...
                        <td>
                          <ul class="orderDatatable_actions mb-0 d-flex flex-wrap">
                            <a href="{{ route('expenses.create') }}?company={{ $paymentReminder->shipping_company_id }}&o_type={{$paymentType}}"
                          ...
                          __('company.Change Payment Date') }}</a>
                            <li>
                              ...
                              <form style="display:none;" id="delete-{{ $paymentReminder->id }}"
                                    action="{{ route('companies.payments.reminder.destroy', [$paymentReminder]) }}" method="POST">
                                @csrf
                                @method('delete')
                              </form>
                            </li>
