            ...
            <div class="userDatatable global-shadow border-light-0 w-100" id="pdf-content">
              <div class="my-3">
                <table class="table mb-0 table-borderless horizontal-table ">
                  ...
                  <tr>
                    <td colspan="2">
                      <form method="post" action="{{route('shipping_companies.statement.update_dollar_rate')}}">
                        @csrf
              ...
              <div class="my-3">
                <table class="table mb-0 table-borderless horizontal-table ">
                  ...
                  <tr>
                    <td>
                      <strong>{{ __('statement.start_balance') . " " . __('statement.yuan') }}</strong> &nbsp;
                      @if($account->starting_balance_rmb >= 0)
                        <strong><span class="text-success">{{ $account->starting_balance_rmb }}</span></strong>
                      @else
                        <strong><span class="text-danger">{{ $account->starting_balance_rmb }}</span></strong>
                      @endif
                    </td>
                    <td>
                  ...
                  @if($account->end_date)
                    <tr>
                      <td><strong>{{ __('statement.current_balance') . " " . __('statement.yuan') }}</strong> &nbsp;
                        <strong><span class="text-success">0</span></strong>
                      </td>
                      <td><strong>{{__('statement.current_balance') . " " . __('statement.dollar') }}</strong> &nbsp;
                        <strong><span class="text-success">0</span></strong>
                      </td>
                    </tr>
                    <tr>
                      <td><strong>{{__('statement.close_balance') . " " . __('statement.yuan') }}</strong> &nbsp;
                        @if($account->total_rmb >= 0)
                          <strong><span class="text-success">{{ $account->total_rmb }}</span></strong>
                        @else
                          <strong><span class="text-danger">{{ $account->total_rmb }}</span></strong>
                        @endif
                      </td>
                      <td><strong>{{__('statement.close_balance') . " " . __('statement.dollar') }}</strong> &nbsp;
              ...
              <div class="table-responsive">
                <table class="table mb-0 table-borderless horizontal-table">
                  ...
                  <tr class="userDatatable-header">
                    ...
                    <th>
                      <span class="userDatatable-title">{{ __('statement.company_payment') }}</span>
                    </th>
                    <th>
                      <span class="userDatatable-title">{{ __('client.balance_dollar') }}</span>
                    </th>
                    <th>
                      <span class="userDatatable-title">{{ __('client.balance_rmb') }}</span>
                    </th>
                    <th>
                  ...
                  @forelse ($ledgers as $date => $ledger_items)
                    @foreach ($ledger_items as $ledger)
                      <tr>
                        ...
                        <td>
                          <div class="userDatatable-content">
                            @if(($ledger['due_rmb'] ?? 0) > -1)
                              <span class="text-success">{{ $ledger['due_rmb'] ?? "-" }}</span>
                            @else
                              <span class="text-danger">{{ $ledger['due_rmb'] ?? "-" }}</span>
                            @endif
                          </div>
                        </td>
                        <td>
                  ...
                  <tr>
                    <td colspan="2">
                      <div class="userDatatable-content">
                        ...
                        <span class="mx-1 {{ $balance_rmb < 0 ?'text-danger':'text-success' }}" id="rmbBalance">
                          {{ $balance_rmb }}
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="schedulePaymentDateModalLabel">{{ __('company.Create Payment Date') }}</h5>
        ...
        <div class="modal-body">
          ...
          <form action="{{ route('companies.payments.reminder.create.date') }}" method="POST">
            ...
            <div class="mb-3">
              <label for="new_payment_date" class="form-label">{{ __('company.Payment Date') }}</label>
              <input type="date" class="form-control" id="new_payment_date" name="new_payment_date" required>
              <input type="hidden" name="shipping_company_id" value="{{$company->id}}">
            </div>

            <!-- Submit button -->
            <button type="submit" class="btn btn-primary">{{ __('company.Save Changes') }}</button>
          </form>
        </div>
      </div>
    </div>
  ...
  <script>
    $(document).ready(function(){
      ...
      account.change(function () {
        if(!$(this).val()) return;
        window.location = "{{route('shipping_companies.statement', request()->id)}}?account=" + $(this).val();
      });
      $(".close-account").click(function () {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.location = "{{route('shipping_companies.statement.restart_account', $company->id)}}";
          }
        });
      });
    });
