...
{{--                    <th>--}}
{{--                      <span class="userDatatable-title">{{trans('client.balance') . " " . trans('statement.rmb')}}</span>--}}
{{--                    </th>--}}
                    <th>
                      <span class="userDatatable-title">{{trans('client.balance') . " " . trans('statement.dollar')}}</span>
                    </th>
                    <th>
...
{{--                        <td>--}}
{{--                          <div class="userDatatable-content">--}}
{{--                            @if(($paymentReminder->client?->activeAccount?->total_rmb ?? 0) > 0)--}}
{{--                              <span class="text-danger fw-bold">--}}
{{--                          {{--}}
{{--                          __(--}}
{{--                          'client.We owe the client :amount RMB'--}}
{{--                          ,['amount'=>number_format(($paymentReminder->client?->activeAccount?->total_rmb ?? 0)--}}
{{--                          *-1, 2, '.', ',')]) }}--}}
{{--                        </span>--}}
{{--                            @elseif(($paymentReminder->client?->activeAccount?->total_rmb ??0) == 0)--}}

{{--                              0.00--}}
{{--                            @else--}}
{{--                              <span class="text-success fw-bold">--}}
{{--                          {{ str_replace('-', '', __('client.The Client owe us :amount RMB',[--}}
{{--                              'amount'=>number_format(abs($paymentReminder->client?->activeAccount?->total_rmb ?? 0),--}}
{{--                          2, '.', ',')])) }}--}}
{{--                        </span>--}}
{{--                            @endif--}}
{{--                          </div>--}}
{{--                        </td>--}}
                        <td>
                          <div class="userDatatable-content">
                            @if(($paymentReminder->client?->activeAccount?->total_dollar ?? 0) > 0)
                              <span class="text-danger fw-bold">
                          {{
                          __(
                          'client.We owe the client :amount dollar'
                          ,['amount'=>number_format(($paymentReminder->client?->activeAccount?->total_dollar ?? 0)
                          *-1, 2, '.', ',')]) }}
                        </span>
                            @elseif(($paymentReminder->client?->activeAccount?->total_dollar ??0) == 0)

                              0.00
                            @else
                              <span class="text-success fw-bold">
                          {{ str_replace('-', '', __('client.The Client owe us :amount dollar',[
                              'amount'=>number_format(abs($paymentReminder->client?->activeAccount?->total_dollar ?? 0),
                          2, '.', ',')])) }}
                        </span>
                            @endif
                          </div>
                       ...
                       d-inline-block w-auto px-2">
                        ...
                        <td>
                          <ul class="orderDatatable_actions mb-0 d-flex flex-wrap">
                            <a class="btn btn-success w-auto mx-2 pay" target="_blank"
                               href="{{ route('expenses.create') }}?client={{ $paymentReminder->client_id }}&o_type={{$paymentType}}">{{
                          __('client.pay_now') }}</a>
                          ...
                          __('client.Change Payment Date') }}</a>
                            <li>
                              ...
                              <form style="display:none;" id="delete-{{ $paymentReminder->id }}"
                                    action="{{ route('clients.claims.reminder.destroy', [$paymentReminder]) }}" method="POST">
                                @csrf
                                @method('delete')
                              </form>
                            </li>
